...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('/check-auth')
        ...
        .then(data => {
            ...
            const userProg = fetch('/get-user-programs', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            .then(programData => {
                ...
                if (programData.programs.length > 0) {
                    ...
                    const availablePrograms = programData.programs.filter(program => program.isAvailable);
                    const unavailablePrograms = programData.programs.filter(program => !program.isAvailable);

         
                    const createButton = (program, isAvailable) => {
                        const button = document.createElement('button');
                        button.id = "programbuttons";
                        button.className = program.name.replace(/\s+/g, '-'); 
                        button.textContent = isAvailable ? program.name : `${program.name} 🔒`;
                            ...
                            button.addEventListener("click", () => {
                                ...
                                .then(result => {
                                    if (result.success) {
                                        ...
                                        alert(result.message || 'Ошибка выбора программы.');
                                    }
                                })
                                .catch(error => {
                    ...
                    availablePrograms.forEach(program => createButton(program, true));
                    unavailablePrograms.forEach(program => createButton(program, false));

                } else {
                    buttonsContainer.innerHTML = '<p>Программы отсутствуют.</p>';
                }
            })
            .catch(error => {
                ...
                Promise.all([fetchProfilePicture, backgroundImage, userProg, getadminfo])
                .then(() => {
